...
def create_vllm_batch_script(
    ...
    job_script.extend([
        ...
        "fi",
        "",
        "# Bind-mount the model path if it's a local directory",
        "MODEL_BIND=\"\"",
        "if [ -d \"$VLLM_MODEL_NAME\" ]; then",
        "    MODEL_BIND=\",$VLLM_MODEL_NAME:$VLLM_MODEL_NAME\"",
        "    echo \"Bind-mounting local model path: $VLLM_MODEL_NAME\"",
        "fi",
        "",
        "VERBOSE=1 apptainer exec --nv --cleanenv \\",
        "    --bind $DATA_INPUT_DIR:/app/data/input,$DATA_OUTPUT_DIR:/app/data/output,$MODELS_DIR:/app/models,$LOGS_DIR:/app/logs,$VLLM_HOME:$VLLM_HOME,$HF_HOME:$HF_HOME,$XDG_CACHE_HOME:$XDG_CACHE_HOME,$FLASHINFER_WORKSPACE_BASE:$FLASHINFER_WORKSPACE_BASE${MODEL_BIND} \\",
