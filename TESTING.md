...
# With coverage report
pytest --cov=aiflux --cov-report=html --cov-report=term-missing
```
...
# With coverage
python -m pytest --cov=aiflux --cov-report=term-missing
```
...
pytest --cov=aiflux --cov-report=html
...
pytest --cov=aiflux.cli --cov-report=term-missing

# Coverage for multiple modules
pytest --cov=aiflux.cli --cov=aiflux.slurm --cov-report=term-missing
```
...
module load anaconda3 && conda activate aiflux && python -m pytest tests/test_cli.py -v
...
```bash
# One-line command (with anaconda3 module)
module load anaconda3 && conda activate aiflux && pytest tests/test_cli.py -v
...
# Run with coverage
pytest --cov=aiflux --cov-report=term
...
```bash
# If tests can't find aiflux module
...
# Save and run
aiflux run --model llama3.2:3b --input data/prompts.jsonl --output results.json
```
...
# Run with coverage
python -m pytest --cov=aiflux --cov-report=term
...
module load anaconda3 && conda activate aiflux && python -m pytest -v
```
...
# Run on server with module load
module load anaconda3 && conda activate aiflux && pytest -v
```
...
module load anaconda3 && conda activate aiflux && \
aiflux run \
  --account your-account \
  --model llama3.2:3b \
  --input data/prompts.jsonl \
